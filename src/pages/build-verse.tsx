...
const BuildVersePage: NextPage = () => {
  ...
  const {
    data: depositData,
    error: depositLoadError,
    isLoading: isDepositLoading,
  } = useL1BuildDeposit(isLegacy);
  ...
  return (
    <div className='space-y-10 grid grid-cols-8 text-sm md:text-base lg:text-lg xl:text-xl lg:text-lg'>
      {ownerAddress && (isDepositLoading || isVerseInfoLoading) && (
        <LoadingModal />
      )}
      ...
      <BuildVerse className='space-y-0.5 col-span-4 col-start-3' />
...
export default dynamic(() => Promise.resolve(BuildVersePage), { ssr: false });
