...
export const Table = (props: Props) => {
  const { heads, records, className } = props;

  return (
    <div style={{ maxHeight: 'calc(2.5rem * 10)', overflow: 'auto' }}>
      <table
        className={clsx(className, 'border-collapse w-full overflow-x-auto')}
      >
        <thead>
              ...
              <tr key={index} className='bg-green-200'>
                {record.map((data, dataIndex) => {
                  return (
                    <td
                      key={dataIndex}
                      className='border px-4 py-2 text-left text-sm font-normal'
                    >
                      {data}
                    </td>
                  );
